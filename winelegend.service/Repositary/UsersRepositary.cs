...
using winelegend.passwordmanager;
    ...
    public class UsersRepositary : IUserService
    {
        ...
        public Users AuthenticateUser(string username, string password)
        {
            var user = _dbContext.Users.Where(x => x.UserName == username || 
                                              x.Email == username || 
                                              x.PhoneNumber == username).FirstOrDefault();
            if (user != null)
            {
                bool isPasswordValid = PasswordHashSalt.VerifyPassword(password, user.PasswordHash, user.PasswordSalt);
                if(isPasswordValid)
                {
                    return user;
                }
